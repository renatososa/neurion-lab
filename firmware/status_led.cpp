...
static const uint32_t ERROR_BLINK_PERIOD_MS = 500;
...
void statusLed_setState(DeviceState state) {
    ...
    switch (state) {
        ...
        case STATE_WIFI_CONNECTED:    setColor(255, 165, 0);   break; // naranja sólido
        case STATE_CONNECTIVITY:      blinkR = 0; blinkG = 0; blinkB = 128; setColor(blinkR, blinkG, blinkB); break; // azul parpadeo
        case STATE_BATTERY_LOW:       blinkR = 128; blinkG = 64;  blinkB = 0; setColor(blinkR, blinkG, blinkB); break; // naranja tenue
        case STATE_ERROR:             blinkR = 128;  blinkG = 0;   blinkB = 0; setColor(blinkR, blinkG, blinkB); break; // rojo muy tenue
    }
}
...
void statusLed_tick() {
    ...
    if (currentLedState == STATE_ERROR) {
        if (now - lastBlinkMs >= ERROR_BLINK_PERIOD_MS) {
            lastBlinkMs = now;
            blinkOn = !blinkOn;
